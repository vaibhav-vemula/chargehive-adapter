...
// Get pending booking for adapter
export async function getPendingBooking(adapterId) {
  const script = `
  ...
  try {
    const booking = await fcl.query({
      cadence: script,
      args: (arg, t) => [arg(adapterId, t.String)]
    });
...
export async function startSession(bookingId) {
  ...
  try {
    ...
    const transactionId = await fcl.mutate({
      cadence: transaction,
      args: (arg, t) => [arg(bookingId, t.UInt64)],
    ...
    const sessionEvent = tx.events.find(e => e.type.includes("SessionStarted"));
    const sessionId = sessionEvent?.data?.sessionId || null;
...
// End session and distribute rewards
export async function endSession(sessionId, energyUsed) {
  const transaction = `
    import CHAdapter from 0xCHAdapter
  ...
  try {
    ...
    const transactionId = await fcl.mutate({
      ...
      args: (arg, t) => [
        ...
        arg(energyUsed.toFixed(2), t.UFix64)
      ],
      proposer: authorization,
      payer: authorization,
      authorizations: [authorization],
    ...
    // Extract reward info from events
    const sessionEndedEvent = tx.events.find(e => e.type.includes("SessionEnded"));
    const rewardsEvent = tx.events.find(e => e.type.includes("RewardsDistributed"));

    return {
      transactionId,
      status: tx.status,
      sessionData: sessionEndedEvent?.data || null,
      rewardsData: rewardsEvent?.data || null,
      events: tx.events
    };
  } catch (error) {
